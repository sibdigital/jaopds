...
@Component({
  selector: 'app-purpose-criteria-view',
  templateUrl: './purpose-criteria-view.component.html',
  styleUrls: ['./purpose-criteria-view.component.scss']
})
    ...
    if (purposeCriteriaMonthlyExecutions) {
      ...
      if (pcmes) {
        ...
        pcmes.forEach(monthlyExecution => {
          ...
          node.push({
            name: moment(month, 'M').format('MMMM').toLocaleUpperCase() + "; Значение: " + monthlyExecution.factPrognos,
          })
